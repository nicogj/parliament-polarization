...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', default='', type=str, help='which legislature or year?')
    ...
    if args.country_code=='fr':
        votes = pd.read_csv(args.data_path + 'france_assemblee_voting/legislature_{}_vote_outcome.tsv'.format(args.legislature_num), sep='\t')

    elif args.country_code=='us' and args.chamber == 'senate':
        votes = pd.read_csv(args.data_path + 'us_senate_voting/congress_{}_1_vote_outcome.tsv'.format(args.legislature_num), sep='\t')
        votes['vote_num'] = "1_" + votes['vote_num'].astype(str)
        if os.path.exists(args.data_path + 'us_senate_voting/congress_{}_1_vote_outcome.tsv'.format(args.legislature_num)):
            votes_2 = pd.read_csv(args.data_path + 'us_senate_voting/congress_{}_2_vote_outcome.tsv'.format(args.legislature_num), sep='\t')
